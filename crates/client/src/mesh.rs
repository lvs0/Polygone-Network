...
/// Build the announce packet for a node + its relay address.
pub fn announce_packet(node_id: &str, relay: &str) -> String {
    format!("POLYGONE v1 {node_id} {relay}\n")
}
...
pub async fn announce(node_id: &str, relay: &str) -> Result<()> {
    ...
    let packet = announce_packet(node_id, relay);
...
pub fn print_peers(peers: &[Peer]) {
    ...
    for p in peers {
        println!("    · {}  →  relay {}", p.node_id, p.relay);
    }
}
...
mod tests {
    ...
    fn announce_packet_format() {
        let p = announce_packet("abc123", "192.168.1.5:7000");
        assert!(p.starts_with("POLYGONE v1 abc123 192.168.1.5:7000"));
    }
}
