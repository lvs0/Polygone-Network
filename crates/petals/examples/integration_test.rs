...
use polygone_petals::types::{InferenceRequest, ChatRequest, ChatMessage, GenerationConfig};
...
async fn main() -> anyhow::Result<()> {
    ...
    for m in &models {
        let size = m.size_gb.map(|s| format!("{:.1} GB", s)).unwrap_or("unknown".to_string());
        println!("  - {} ({})", m.name, size);
    }
    ...
    println!("   Tokens: {}, Time: {}ms, Tokens/s: {:.1}",
