...
impl VllmBackend {
            ...
            .map(|arr| {
                arr.iter().filter_map(|m| {
                    ...
                    Some(ModelInfo {
                        ...
                        quantization: None,
                        context_window: m.get("context_window").and_then(|c| c.as_u64()).map(|c| c as u32),
                        size_gb: None,
    ...
    pub async fn generate(&self, request: InferenceRequest) -> Result<InferenceResponse> {
        ...
        let text = choice.and_then(|c| c.get("text")).and_then(|t| t.as_str()).unwrap_or("").to_string();
        let tokens = data.get("usage").and_then(|u| u.get("completion_tokens")).and_then(|t| t.as_u64()).unwrap_or(0) as u32;
        let prompt_tokens = data.get("usage").and_then(|u| u.get("prompt_tokens")).and_then(|t| t.as_u64()).unwrap_or(0) as u32;
        ...
        Ok(InferenceResponse {
            ...
            finish_reason: choice.and_then(|c| c.get("finish_reason")).and_then(|r| r.as_str()).map(|s| s.to_string()),
    ...
    pub async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let mut req = self.client.post(self.url("/v1/chat/completions")).json(&payload);
        if let Some(auth) = self.auth_header() {
            req = req.header("Authorization", auth);
        }
        ...
        let choice = data.get("choices").and_then(|c| c.as_array()).and_then(|a| a.first());
        let message = choice.and_then(|c| c.get("message")).cloned().unwrap_or_default();
        let content = message.get("content").and_then(|c| c.as_str()).unwrap_or("").to_string();
        let tool_calls = message.get("tool_calls").and_then(|t| serde_json::from_value(t.clone()).ok());
