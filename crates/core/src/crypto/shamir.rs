...
/// A unique fragment identifier in `1..=n`.
...
mod tests {
            ...
            for j in (i + 1)..7 {
                for k in (j + 1)..7 {
                    for l in (k + 1)..7 {
                        ...
                        let recovered = reconstruct(&subset, 4).unwrap();
                        assert_eq!(recovered, secret);
                    }
                }
            }
    ...
    #[test]
    fn shamir_empty_secret_cannot_reconstruct() {
        // sharks 0.5 shares are always >= 2 bytes (x + y). An empty secret
        // yields 1-byte shares that the library refuses to recover.
        // Documenting the constraint rather than the math.
        let frags = split(b"", 4, 7).unwrap();
        assert!(reconstruct(&frags[..4], 4).is_err());
    }
}
